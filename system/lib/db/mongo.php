<?php  namespace core\db;
use core\arr;
/**
* @author Sun <sunkang@wstaichi.com>
* @license  
 */
/*
 * mongo db数据库操作
 * 查寻手册 
 * http://php.net/manual/en/mongocollection.findone.php
 * 
 *
 */

class  mongo{
	protected $db;
	protected $collection;
	
	static $server;
	static $options;

	protected $client;

	function __construct(){
		$server = static::$server;
		$options = static::$options;
		$db = $options['db'];
		unset($options['db']);
		$this->client = new \MongoClient($server , $options); 
		$this->db = $this->client->selectDB($db);
		$this->db->setReadPreference(\MongoClient::RP_NEAREST, array(
		    array('dc' => 'east', 'use' => 'reporting'),
		    array('dc' => 'west', 'use' => 'reporting'),
		));
	}	

	function log(){
		$pr = 'system.profile';
		$r = $this->db->$pr->find()->sort( ['ts'=>-1] );
		foreach($r as $v){
			$vo['ns'] = $v['ns'];
			$vo['query'] = $v['query'];
			$vo['op'] = $v['op'];
			$vo['client'] = $v['client'];
			$vo['ts'] = $v['ts'];
			$vo['millis'] = $v['millis'].'毫秒';
			$vs[] = $vo;
		}
		
		return $vs;
	}
	
	
	function insert($data = []){ 
		$keys = array_keys($data);
		if(!$keys){
			$keys = array_keys($collection);
		}

		$flag = true;
		foreach($keys as $v){
			if(!is_numeric($v)){
				$flag = false;
			}
		}
		if($flag === true){
			 $this->collection->batchInsert($data);
			 return $data;
		}else{
			 $this->collection->insert($data);
			 return (string)$data['_id'];
		}
	}
	 
	
	function updateAll($condition,$set_or_inc = [],$par = [] ){
			$par['multiple'] = true;
			return $this->update($condition,$set_or_inc,$par);

	 
	}
	function update($condition,$set_or_inc = [],$par = array('multiple'=>false)){
			if(strpos(arr::get($set_or_inc,0)['k'],'$')===false){
				$set_or_inc = ['$set'=>$set_or_inc];
			}
			return $this->collection->update($condition,$set_or_inc , $par);;
	}
	
	
	function table($table = null){
		if($table)
			$this->collection = $this->db->$table;
		return $this;
	}
	
	function collection($table){
		$this->table($table);
		return $this;
	}
	
  	function order_by($order){
  			if(!$order)  return;
  			$o['desc'] = -1;
  			$o['asc'] = 1;
  			if($order){
  				foreach($order as $k=>$v){
  					if($o[$v]) $v = $o[$v];
  					$new[$k] = $v;
  				}	
  				$order = $new;
  			}
  			$this->ar['sort'] = $order;	
  			return $this->collection->sort($order);;
  	} 
 	
	
	function __call($method,$arg = []){ 

		
 	    $call = call_user_func_array([$this->collection, $method],$arg);
 	    
 	    return $call;
    }
	
	
	
}



