...
class plupload extends base{
	...
	function run(){
		...
		if($data){
			foreach ($data as $vo){
				$dataString .=  "<div class='pluploadCkeditor file_upload_div'>
		...
		$this->script[] = "
			...
			".$var." .bind('FileUploaded', function(up,file,response) {
				...
				if(res.error){
					...
					var img = \"<img class='img-thumbnail' style='5px 5px 0px 0px;width:200px;height:200px;' src='\"+res.path_thumb+\"'/>\";

					var html = \"<div class='pluploadCkeditor file_upload_div'><span class='glyphicon glyphicon-remove pluploadRemoveFile'></span>\"+img+\"<input type='hidden' name='".$this->ele."[]' value='\"+res.path+\"' ></div>\";
